...
ComponentManagerRT::ComponentManagerRT(
  ...
  executor_(executor)
{
  ...
  loadNode_srv_ = create_service<LoadNode>(
    ...
    std::bind(&ComponentManagerRT::on_load_node, this, _1, _2, _3),
    service_qos);
  unloadNode_srv_ = create_service<UnloadNode>(
    "~/_container/unload_node",
    std::bind(&ComponentManagerRT::on_unload_node, this, _1, _2, _3),
    service_qos);
  listNodes_srv_ = create_service<ListNodes>(
    "~/_container/list_nodes",
    std::bind(&ComponentManagerRT::on_list_nodes, this, _1, _2, _3));

  {
    this->declare_parameter("sched_priority", rclcpp::PARAMETER_INTEGER);
    // Get node names that has tracing enabled
    this->declare_parameter("tracing_node_names", rclcpp::PARAMETER_STRING_ARRAY);
    tracing_node_names_ = this->get_parameter("tracing_node_names").as_string_array();
    // get trace file path
    this->declare_parameter("trace_file_path", rclcpp::PARAMETER_STRING);
    trace_file_path_ = this->get_parameter("trace_file_path").as_string();
    set_trace_file_and_start_tracing();
  }
}
...
ComponentManagerRT::get_component_resources(
  const std::string & package_name, const std::string & resource_index) const
{
  ...
  {
    throw ComponentManagerRTException("Could not find requested resource in ament index");
  }
  ...
  for (const auto & line : lines) {
    ...
    if (parts.size() != 2) {
      throw ComponentManagerRTException("Invalid resource entry");
    }
...
std::shared_ptr<rclcpp_components::NodeFactoryRT>
ComponentManagerRT::create_component_factory(const ComponentResource & resource)
{
  ...
  if (loaders_.find(library_path) == loaders_.end()) {
    ...
    try {
      ...
      throw ComponentManagerRTException("Failed to load library: " + std::string(ex.what()));
    } catch (...) {
      throw ComponentManagerRTException("Failed to load library");
    }
  }
  loader = loaders_[library_path].get();

  auto classes = loader->getAvailableClasses<rclcpp_components::NodeFactoryRT>();
  for (const auto & clazz : classes) {
    ...
    if (clazz == class_name || clazz == fq_class_name) {
      ...
      return loader->createInstance<rclcpp_components::NodeFactoryRT>(clazz);
    }
  }
  return {};
}
...
rclcpp::NodeOptions
ComponentManagerRT::create_node_options(const std::shared_ptr<LoadNode::Request> request)
{
  ...
  for (const auto & a : request->extra_arguments) {
    ...
    if (extra_argument.get_name() == "use_intra_process_comms") {
      if (extra_argument.get_type() != rclcpp::ParameterType::PARAMETER_BOOL) {
        throw ComponentManagerRTException(
...
void
ComponentManagerRT::set_trace_file_and_start_tracing()
{
  if(trace_file_path_.empty()){
    RCLCPP_ERROR(get_logger(), "No trace file path has been set as Component Manager RT parameter. " 
...
void
ComponentManagerRT::set_executor(const std::weak_ptr<rclcpp::Executor> executor)
{
  executor_ = executor;
}
...
void
ComponentManagerRT::remove_node_from_executor(uint64_t node_id)
{
  if (auto exec = executor_.lock()) {
...
void
ComponentManagerRT::on_load_node(
  ...
  std::shared_ptr<LoadNode::Response> response)
{
  ...
  try {
    ...
    for (const auto & resource : resources) {
      ...
      try {
        ...
        // In the case that the component constructor throws an exception,
        // rethrow into the following catch block.
        throw ComponentManagerRTException("Component constructor threw an exception");
      }
  ...
  } catch (const ComponentManagerRTException & ex) {
...
void
ComponentManagerRT::on_unload_node(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
void
ComponentManagerRT::on_list_nodes(
  const std::shared_ptr<rmw_request_id_t> request_header,
